...
初版实现用 `edgeSpillThreshold + 100` 作为处理区间外边界，代码审查用真实调用参数
（白色 chroma，`seedThreshold=30, edgeSpillThreshold=38`）和仓库自带的肤色测试夹具
`#f3d3b1`（色距 ≈90.5）实测发现：`+100` 会把这类"贴边但本身就是纯色、并非抗锯齿
过渡色"的肤色像素也当作溢色处理，产生新的半透明+变色缺陷——用消灭白边的手段制造
了肤色色斑。

修正为 `edgeSpillThreshold × 2`（白色阈值下=76，绿色阈值下=140），实测能把
`#f3d3b1` 排除在处理区间之外（76 < 90.5），同时仍能处理色距更小、更贴近真实抗
锯齿过渡的像素（如 25% 混合的过渡像素，色距 63.8，落在处理区间内）。已补充对应
回归用例（`scripts/verify/verify-chroma-decontaminate.mjs` Case 2.5）防止后续调参
时再次引入这类误伤。

代价：处理区间变窄后，线性色距估计对"真实混合比例低、但前景本身饱和度高（离
chroma 色距天然很大）"的像素颜色还原精度会下降（这类估计的固有误差来源见函数内
注释）。这是在"消除白边"和"不误伤贴边浅色前景"之间的权衡取舍，倾向保守。
